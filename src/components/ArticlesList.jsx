  ...
  return (
    <>
      <animated.div style={fadeIn} className='body_category'>
          ...
          <MDBIcon onClick={()=>props.setDisplay(-1)} style={{marginBottom: '50px', cursor: 'pointer'}} fas size="2x" icon="arrow-circle-left" />
            ...
            {props.data[props.display].articles.map((article)=>(
              <div key={article.name} className="article">
                <div className="article_container">
                  <div>
                  <span className="article_name" style={{marginRight: '10px'}}>{article.name}</span>
                  
                  {article.image && <MDBIcon fas icon="camera" size="sm" style={{cursor: 'pointer'}} onClick={()=>showModal(article.image)}/> } 
                  </div>
